class Myr {
    constructor(canvasElement, _gl) {

        _gl.pixelStorei(_gl.UNPACK_ALIGNMENT, 1) // add support for non 4 byte aligned textures
        ...
        Renderable.prototype = {
            ...
            drawYFlipped: function (x, y, xScale = 1, yScale = 1) {
                this._prepareDraw()
        ...
        this.Surface = function () {
            let _mode = RENDER_MODE_SURFACES
            ...
            this.uploadData = (x, y, array, offset) => {
                _gl.activeTexture(TEXTURE_EDITING)
                _gl.bindTexture(_gl.TEXTURE_2D, _texture)
                _gl.texSubImage2D(_gl.TEXTURE_2D, 0, x, y, array.length, 1, _gl.LUMINANCE, _gl.UNSIGNED_BYTE, array, 0)
            }
            ...
            if (typeof arguments[0] === "boolean") {
                ...
                _height = arguments[2]
                _ready = true
                _mode = RENDER_MODE_COLORING
                _gl.texImage2D(
                    _gl.TEXTURE_2D, 0, _gl.LUMINANCE, _width, _height, 0, _gl.LUMINANCE, _gl.UNSIGNED_BYTE,
                    null)
            }
        ...
        const flush = this.flush = (noclear) => {
            if (_instanceCount === 0)
                return
        ...
        this.transformForce = transform => {
            _transformAt = 0
            touchTransform().set(transform)
        }
        ...
        const RENDER_MODE_SHADER = 7
        const RENDER_MODE_COLORING = 6
        ...
        const _shaderCoreColoring = new ShaderCore(
            "layout(location=0) in mediump vec2 vertex;" +
            "layout(location=1) in mediump vec4 a1;" +
            "layout(location=2) in mediump vec4 a2;" +
            "layout(location=3) in mediump vec4 a3;" +
            ...
            color = vec4((value - 0.5) * 2.0, 0.0, (1.0 - value) * 2.0, 1.0);
            `+ "}"
        )
...
Myr.Transform = class {
    constructor(_00, _10, _20, _01, _11, _21) {
        if (_00 === undefined)
            this.identity()
        else {
            this._00 = _00
